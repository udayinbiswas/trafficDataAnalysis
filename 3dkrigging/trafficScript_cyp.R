...
train_data$cyp <- train_label$Car...Jeep..Commercial.Yellow.plate.
...
vehiclesDF <- data.frame(Cyp=vehicles.UTM$cyp)
print('create STIDF')
# Merge
timeDF <- STIDF(vehiclesSP,vehiclesTM,data=vehiclesDF) 
print('creating variogram')
# Variogram
var <- variogramST(Cyp~1,data=timeDF,tunit="mins",tlags=5*(0:72),assumeRegular=F,na.omit=T)
print('done')
# Save the data
save.image(file="var_cyp.RData")