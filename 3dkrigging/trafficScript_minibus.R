...
train_data$minibus <- train_label$minibus
...
vehiclesDF <- data.frame(minibus=vehicles.UTM$minibus)
print('create STIDF')
# Merge
timeDF <- STIDF(vehiclesSP,vehiclesTM,data=vehiclesDF) 
print('creating variogram')
# Variogram
var <- variogramST(minibus~1,data=timeDF,tunit="mins",tlags=5*(0:72),assumeRegular=F,na.omit=T)
print('done')
# Save the data
save.image(file="var_minibus.RData")