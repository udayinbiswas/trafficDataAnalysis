...
train_data$X2.w <- train_label$X2.w
...
vehiclesDF <- data.frame(TwoWheeler=vehicles.UTM$X2.w)
print('create STIDF')
# Merge
timeDF <- STIDF(vehiclesSP,vehiclesTM,data=vehiclesDF) 
print('creating variogram')
# Variogram
var <- variogramST(TwoWheeler~1,data=timeDF,tunit="mins",tlags=5*(0:72),assumeRegular=F,na.omit=T)
print('done')
# Save the data
save.image(file="var_2_w.RData")