...
train_data$hcv <- train_label$heavy.commercial.vehicles
...
vehiclesDF <- data.frame(Hcv=vehicles.UTM$hcv)
print('create STIDF')
# Merge
timeDF <- STIDF(vehiclesSP,vehiclesTM,data=vehiclesDF) 
print('creating variogram')
# Variogram
var <- variogramST(Hcv~1,data=timeDF,tunit="mins",tlags=5*(0:72),assumeRegular=F,na.omit=T)
print('done')
# Save the data
save.image(file="var_hcv.RData")